        ...
        <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:navlist variant="outline">
                <flux:navlist.group :heading="__('Plataforma')" class="grid">
                    ...
                    <flux:navlist.item icon="calendar-days" :href="route('mis-talleres')" :current="request()->routeIs('mis-talleres')" wire:navigate>{{ __('Mis Talleres') }}</flux:navlist.item>
                    @php $user = auth()->user(); @endphp
                    @if($user && $user->isAdmin())
                        <flux:navlist.item icon="wrench-screwdriver" :href="route('admin.talleres')" :current="request()->routeIs('admin.talleres')" wire:navigate>Gestión de Talleres</flux:navlist.item>
